...
from ..models import Faction, MinionCard, ActionCard, ScrapingResult
...
class FactionScraper(BaseScraper):
    ...
    def scrape_faction_data(self, faction_name: str, set_id: str) -> Faction:
        ...
        faction_url = (f"{self.web_client.BASE_URL}{faction_name}" 
    ...
    def scrape_faction_cards(self, faction_name: str, faction_id: str) -> List[MinionCard | ActionCard]:
        """
        ...
        if result.success:
            self._log_scraping_complete("card scraping", result.items_processed, faction_name)
            ...
            logger.error(f"Failed to scrape cards for faction {faction_name}: {result.message}")
            return []
    ...
    def scrape(self, faction_name: str, set_id: str = None) -> ScrapingResult:
        ...
        try:
            ...
            if not set_id:
                ...
                logger.warning(f"No set_id provided for faction {faction_name}, using generated ID")
            ...
            self._log_scraping_complete("faction scraping", 1 + len(cards), faction_name)
